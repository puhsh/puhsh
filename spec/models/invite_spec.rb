...
describe Invite do
  ...
  describe '.create_multiple' do
    ...
    it 'creates a single invite' do
      Invite.create_multiple([{user_id: user.id, uid_invited: '1234567'}])
      expect(user.reload.invites.collect(&:uid_invited)).to include('1234567')
    end

    it 'creates multiple invites' do
      Invite.create_multiple([{user_id: user.id, uid_invited: '1234567'}, {user_id: user.id, uid_invited: '654321'}])
      expect(user.reload.invites.collect(&:uid_invited)).to include('1234567')
      expect(user.reload.invites.collect(&:uid_invited)).to include('654321')
    end
